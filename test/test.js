...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof itermprod, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a moving product (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = itermprod( createIterator( values ), 3 );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	itermprod = proxyquire( './../lib/main.js', {
